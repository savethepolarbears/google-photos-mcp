...
export class GooglePhotosMCPCore {
    ...
    return {
      ...
      resourceTemplates: [
        ...
        },
        {
          ...
          description:
            "A specific Google Photos media item by ID. Note: baseUrl is ephemeral (~60 min); always fetch fresh.",
          mimeType: "application/json",
        },
      ],
    };
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              fileName: {
                type: "string",
                description: "File name to use in Google Photos",
              },
              albumId: {
                type: "string",
                description:
                  "Optional: album ID to add the media to immediately",
              },
              description: {
                type: "string",
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              albumId: {
                type: "string",
                description: "ID of the album to add media to",
              },
              mediaItemIds: {
                type: "array",
                items: { type: "string" },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              dates: {
                type: "array",
                description: "Specific dates to filter by (max 5)",
              },
              dateRanges: {
                type: "array",
                description: "Date ranges to filter by (max 5)",
              },
              includedCategories: {
                type: "array",
              ...
              excludedCategories: {
                type: "array",
                description: "Content categories to exclude",
              },
              mediaType: {
                type: "string",
                enum: ["ALL_MEDIA", "PHOTO", "VIDEO"],
                description: "Media type filter",
              },
              includeFavorites: {
                type: "boolean",
                description: "Include only favorites",
              },
              includeArchived: {
                type: "boolean",
              ...
              orderBy: {
                type: "string",
                description: "Order results (requires dateFilter)",
              },
              pageSize: {
                type: "number",
...

        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              locationName: {
                type: "string",
                description: "Location name (required when type is LOCATION)",
              },
              latitude: {
                type: "number",
                description: "Latitude for location enrichment",
              },
              longitude: {
                type: "number",
                description: "Longitude for location enrichment",
              },
              position: {
                type: "string",
                enum: ["FIRST_IN_ALBUM", "LAST_IN_ALBUM"],
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              mediaItemId: {
                type: "string",
                description: "ID of the media item to use as cover",
              },
            },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              albumTitle: {
                type: "string",
                description: "Title for the new album (1-500 chars)",
              },
              files: {
                type: "array",
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              sessionId: {
                type: "string",
                description: "The Picker session ID from create_picker_session",
              },
              pageSize: {
                type: "number",
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              message:
                "Not authenticated with Google Photos. Please authenticate first.",
            }),
          },
        ],
      };
  ...
  private async handleStartAuth() {
    ...
    const { parseIdToken, resolveUserIdentity } =
      await import("../utils/googleUser.js");
    ...
    app.get(
      ...
      async (
        ...
        try {
          ...
          const verifiedPayload = await parseIdToken(
            tokens.id_token,
            oauth2Client,
          );
    ...
    try {
      ...
      const pids = execSync(`lsof -i :${port} -P -n -t 2>/dev/null`, {
        encoding: "utf8",
      })
        .trim()
        .split("\n")
      ...
      if (pids.length > 0) {
        ...
        logger.info(
          `Killed ${pids.length} orphaned process(es) on port ${port}`,
        );
      }
    } catch {
    ...
    const httpServer = await new Promise<http.Server>((resolve, reject) => {
      ...
      server.on("error", (err: NodeJS.ErrnoException) => {
        if (err.code === "EADDRINUSE") {
          reject(
            new Error(
              `Port ${port} is already in use. Please close any other servers on port ${port} and try again.`,
            ),
          );
        } else {
          reject(err);
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              message:
                "Authentication server started. Open the URL below in your browser to authenticate with Google Photos.",
              authUrl,
              instructions: [
  ...
  private async handleAddMediaToAlbum(
  ...
  private async handleSearchPhotos(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
  ...
  private async handleSearchPhotosByLocation(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
  ...
  private async handleListAlbumPhotos(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
  ...
  private async handleListMediaItems(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
    const args = validateArgs(request.params.arguments, listAlbumsSchema);
  ...
  private async handleSearchMediaByFilter(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
    const args = validateArgs(
    ...
    const { photos, nextPageToken } = await searchPhotos(oauth2Client, {
      filters,
  ...
  private async handleAddAlbumEnrichment(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
  ...
  private async handleCreateAlbumWithMedia(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
    const args = validateArgs(
    ...
    for (const file of args.files) {
      try {
        ...
        uploadResults.push({
          ...
          success: true,
          mediaItemId: (media as { mediaItemId?: string }).mediaItemId,
        });
      } catch (err) {
        uploadResults.push({
          fileName: file.fileName,
    ...
    const addedCount = uploadResults.filter(
      (r) => r.success && r.mediaItemId,
    ).length;
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { album, uploadResults, addedToAlbum: addedCount },
            null,
            2,
          ),
        },
      ...
      prompts: [
        {
          ...
          description:
            "Guides you through organizing photos into albums by theme or date range.",
          arguments: [
            {
              name: "theme",
              description: 'Theme or subject (e.g., "vacation", "family")',
              required: false,
            },
            {
              name: "dateRange",
              description: 'Date range (e.g., "2023", "summer 2022")',
              required: false,
            },
          ],
        },
        {
          ...
          description:
            "Step-by-step workflow for uploading multiple local files to a Google Photos album.",
          arguments: [
            {
              name: "albumName",
              description: "Target album name (new or existing)",
              required: false,
            },
          ],
        },
        {
          ...
          description:
            "Constructs a valid Google Photos filter query from a plain-language description.",
          arguments: [
            {
              name: "criteria",
              description:
                'What you want to find (e.g., "pet photos from last year")',
              required: true,
            },
          ],
        },
...
Error handling: If an upload fails, continue with remaining files and report partial results.`,
      ...
      case "find_photos_by_criteria":
        return {
          description:
            "Guide for constructing valid Google Photos filter queries.",
          messages: [
            {
              role: "user" as const,
              content: {
...
- For plain-text search, use search_photos instead of filter-based search.`,
  ...
  private handleDescribeFilterCapabilities() {
    const capabilities = {
      ...
      dateFilter: {
        description:
          "Filter by specific dates or date ranges (mutually exclusive)",
        maxDates: 5,
        maxRanges: 5,
        dateFields: {
  ...
  private async handleSetAlbumCover(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
   ...
   */
  private async handlePollPickerSession(
    request: CallToolRequest,
    tokens: TokenData,
  ) {
    const args = validateArgs(
