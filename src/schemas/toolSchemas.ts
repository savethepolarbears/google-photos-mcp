...
export const searchMediaByFilterSchema = z
  .object({
    ...
    dateRanges: z
      ...
      .max(5, "Maximum 5 date ranges allowed")
      .optional(),
  ...
  .refine((data) => !(data.dates && data.dateRanges), {
    message:
      "dates and dateRanges are mutually exclusive — use one or the other",
  })
  .refine((data) => !(data.orderBy && !data.dates && !data.dateRanges), {
...
export const addEnrichmentSchema = z
  ...
  .refine((data) => !(data.type === "LOCATION" && !data.locationName), {
...
export const createAlbumWithMediaSchema = z.object({
  ...
  files: z
    ...
    .max(50, "Maximum 50 files per call"),
});
